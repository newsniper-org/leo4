...
fn parses_lake_plugin_sample_schema() {
    ...
    // `mutual { Expr; Stmt }` cluster, plus the Phase 8 carriers
    // `Leo4.LeanU128` (#55) and `Leo4.LeanComplexF64x2` (#56) — ten
    // entries total.
    assert_eq!(schema.user_decls.len(), 10, "expected 10 user decls");
